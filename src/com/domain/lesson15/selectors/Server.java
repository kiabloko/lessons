...
public class Server {
    ...
    private class NioThread extends Worker{
        ...
        protected void init() throws IOException {
            ...
            // создаем селектор -помогает переключаться  содного канала на другой,если их несколько
            //канал должен быть в селекторе зарегистрирован, селектор следит за каналом
            ...
            // регистрируем канал в селекторе на все допустимые события (для serverSocket - OP_ACCEPT) - в ServerSocketChannel только это событие
            serverChannel.register(selector, serverChannel.validOps()); //метод валидопс возвращает все допустимые события
        }
        //действия в канале,отслеживающиеся селектором:
        //connect OP_CONNECT
        //accept - когда был воспроизведен клиентский сокет OP_ACCEPT
        //read OP_READ
        // write OP_WRITE
